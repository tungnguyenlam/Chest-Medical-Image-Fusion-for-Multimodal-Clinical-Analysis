...
## 2026-06-20 — Stage 04 builds all CXR-LT variants by default; de-hardcode 2023 in stages 01/02

- The shared helper [src/dataloader/cxr_lt.py](src/dataloader/cxr_lt.py) already knew both releases, and stage 04 had `--cxr-lt-*` flags, but the upstream prepare steps hardcoded the CXR-LT 2023 paths/label list, so the pipeline could not produce a 2024 dataset end-to-end without the manual `scripts/relabel_prepared_cxrlt.py` bridge.
- [src/prepare/04_build_prior_aware_dataset.py](src/prepare/04_build_prior_aware_dataset.py): new **build-all-variants default** (no `--cxr-lt-version`). Reads the label-agnostic `03_mimic_*` base once, then for each of the 5 variants (2023/standard, 2024/{all,task1,task2,task3}) re-attaches labels, re-splits per release, writes `<variant>/{train,val,test}.csv` + a parquet trio + `*_label_metadata.json`. Variants with missing label CSVs are skipped with a warning. Refactored `build_split` into a DataFrame core `build_split_df` + thin CSV wrapper; folded the relabel logic in (`read_base_csvs`). Single-variant mode (pass `--cxr-lt-version`) unchanged.
- [src/prepare/01_make_dataset.py](src/prepare/01_make_dataset.py): both label merges now go through `load_cxr_lt_labels('data', 'cxr-lt-2023')` instead of hardcoded paths + a literal 26-label list. Behavior identical (2023 baseline); only the source of truth moved to the helper.
- [src/prepare/02_split_dataset.py](src/prepare/02_split_dataset.py): split id sets derived from the helper's normalized `split` column instead of re-reading the 2023 CSVs.
- Non-obvious: stage 04 re-attaches labels onto the base by `dicom_id`, so the 2023 labels baked by stage 01 are not load-bearing for 2024 — they are just the default. Split filenames differ per release (2023 keeps `development`, 2024 uses `val`).
- Docs: updated [camchex/data/DATASET_FLOW.md](camchex/data/DATASET_FLOW.md), the stage-04 module docstring, and [training/prior_aware_v5nano_cxrlt2024/README.md](training/prior_aware_v5nano_cxrlt2024/README.md). `scripts/relabel_prepared_cxrlt.py` is retained for one-off custom relabels.

  - **Verified** the "same images, only labels/splits differ" assumption that makes the stage-04-only approach correct (no 01/02/03 changes needed): the 2024 dicom_id universe is a subset of the 2023 union (377,110 ids). all/task1/task3 == 377,110 exactly with 0 ids outside 2023; task2 = 298,570, also fully inside. So pruning to the 2023-labeled rows in 01/02/03 drops no 2024 study, and stage 04's per-release re-split + inner-join selects the correct set.

## 2026-06-20 — CXR-LT 2024 (task1) config variant for every model

- Added `training/<model>_cxrlt2024/config.yaml` for all 14 models that lacked one (joining the 2 hand-made variants), so every model can train on the 40-label CXR-LT 2024 task1 space by pointing the base entry point at `--config training/<model>_cxrlt2024/config.yaml`. No new train scripts — each variant reuses its base model's (family-named) train/eval script.
- Each variant changes only four coupled things vs its base (verified by diff): the 40 task1 `classes` list, `num_classes`/`n_classes` → 40, the three `*_df_path`, and the ASL loss `class_instance_nums`/`total_instance_num`.
- **Loss weights are real, not placeholders** (unlike the shipped `prior_aware_v5nano_cxrlt2024` which had `null`):
  - camchex family (6 incl. existing): per-IMAGE counts over `cxr-lt-2024/train_labeled.csv` (total 258,871). Reproduced the existing `camchex_v2nano_vitals_cxrlt2024` numbers exactly as a check.
  - prior_aware family (10 incl. existing): per-STUDY counts (total 329,114) from the build sidecar `prior_aware_cxrlt2024_task1_label_metadata.json`. These are data-only/model-independent, so one build fills all prior_aware variants.
- Non-obvious: stage 04 builds the prior-aware parquet without needing the JPEGs (`resolve_preferred_image_path` only stats, never decodes), so the 2024 parquets + per-study weight metadata were generated on this data-but-no-images box. Ran the single-variant build to produce `data/data-camchex/prior_aware_cxrlt2024_task1_{train,val,test}.parquet` (+ sidecar).
- Generator (one-off, scratchpad): ruamel round-trip with `indent(mapping=2, sequence=4, offset=2)` + explicit-`null` representer so output matches the repo's hand-written config style; only the four intended keys differ from each base. Train-command in each config header is resolved from the actual train script in the base dir (they're family-named, e.g. `prior_aware_*` all ship `prior_aware_train.py`).

## 2026-06-20 — Eval 2024 (task1/all) models on the CXR-LT 2024 task2 gold set

- A model trained on a 2024 label set that is a superset of the 26 task2 labels (task1/all) is now, by default, ALSO scored on the task2 (gold) test set during eval — its wide outputs are sliced to the 26 task2 columns (task2 ⊂ task1) and scored against the small expert-labeled gold set. Written to `*_task2_gold.{csv,json}` next to the normal metrics. `--skip-task2-gold` opts out; auto-skipped for CXR-LT 2023 models (their label set isn't a task2 superset).
- [training/utils/evaluation.py](training/utils/evaluation.py): `predict_dataframe` gained `output_indices` (slice model outputs to a label subset); new `maybe_evaluate_cxrlt2024_task2_gold` does the gold pass; `evaluate_report_ablation` gained an optional `cfg` param and calls it. The gold loader is built by mutating `args.test_df_path` (→ task2 path) + `cfg["model"]["classes"]` (→ 26 task2) and re-invoking the existing `make_loader` closure, then restoring — so no per-family loader plumbing was needed.
- Wiring: added `cfg=cfg` to the `evaluate_report_ablation(...)` call in all 15 `*_eval.py` scripts; `singleview_eval.py` (different eval flow) calls the helper directly with a `[::2]` adapter over its `(loader, ids, labels_available)` factory. New eval-only flag `--skip-task2-gold` in [training/common.py](training/common.py).
