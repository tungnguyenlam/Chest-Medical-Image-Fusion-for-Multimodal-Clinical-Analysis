...
class CaMCheXV3NanoModel(nn.Module):
    """ConvNeXtV2 Nano + CXR-BERT + numeric vitals with one token per non-image modality."""
    ...
    def forward(self, data):
        ...
        clin_token = (clinical_cls + clin_seg).unsqueeze(1)
        ...
        tokens = torch.cat([img_tokens, clin_token, vital_tokens], dim=1)
        ...
        text_valid_tokens = torch.ones(b, 2, dtype=torch.bool, device=tokens.device)
