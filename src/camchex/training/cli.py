...
_LEGACY_CAMCHEX_DIR = _REPO_ROOT / "camchex"


def _resolve_cli_path_args() -> None:
    path_flags = {"--config", "--ckpt_path"}
    for i, arg in enumerate(sys.argv):
        if arg in path_flags and i + 1 < len(sys.argv):
            p = Path(sys.argv[i + 1])
            if not p.is_absolute():
                sys.argv[i + 1] = str((_ORIG_CWD / p).resolve())
        else:
            for flag in path_flags:
                prefix = f"{flag}="
                if arg.startswith(prefix):
                    p = Path(arg[len(prefix):])
                    if str(p) and not p.is_absolute():
                        sys.argv[i] = prefix + str((_ORIG_CWD / p).resolve())
                    break


_resolve_cli_path_args()

# Keep dataset/image paths compatible with the original paper code. CSV image
# paths such as images/p10/... are relative to the legacy camchex/ directory.
os.chdir(_LEGACY_CAMCHEX_DIR)

import torch
import yaml
from lightning.pytorch.cli import LightningCLI

from camchex.data.datamodule import CaMCheXDataModule
from camchex.training.lightning_module import CaMCheX
...
def _configure_run_paths(cfg_node):
    ...
    _update_class_path_callback(
        callbacks,
        "camchex.callbacks.prediction_callback.PredictionWriter",
        {"output_path": str(Path(run_dir) / "predictions" / "predictions.csv")},
    )
    _update_class_path_callback(
        callbacks,
        "camchex.callbacks.run_logger.RunLoggerCallback",
        {
            "run_dir": run_dir,
