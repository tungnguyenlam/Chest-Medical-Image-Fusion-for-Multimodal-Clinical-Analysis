...
In the active refactored implementation, the text encoder is changed to CXR-BERT-specialized (\texttt{microsoft/BiomedVLP-CXR-BERT-specialized}). CXR-BERT is designed for chest X-ray language understanding and is further specialized using radiology reports and multimodal image--text contrastive training in the BioViL framework \cite{boecking2022biovil}. This makes it more closely matched to the language distribution of MIMIC-CXR reports and clinical indications than a general biomedical encoder. The change is therefore motivated by domain alignment rather than by architectural novelty.
