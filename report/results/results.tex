...
We evaluate all implemented models on the filtered CXR-LT-2023-aligned test split. This section reports the experimental setup, main quantitative results, ablation studies, long-tail analysis, and qualitative Grad-CAM checks.
...
The final model was trained as the selected full configuration. However, a complete factorial ablation would require many additional full training runs, because design choices such as prior context, time-gap embedding, prior dropout, text-encoder fine-tuning, label-correlation modelling, channel encoding, and random seeds would need to be isolated independently. Due to the available computational budget, we prioritize ablations that directly answer the main research question. Therefore, the modality-ablation rows should be interpreted as controlled comparisons, while the incremental-variant rows are reported as development evidence rather than a complete causal decomposition.
